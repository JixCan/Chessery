...
export default function PuzzlesPage() {
    ...
    const handleEvent = useCallback((orig, dest) => {
        console.log(orig, dest);
        let move = orig + dest;  // Формируем ход в формате 'e2e4'
        
        // Ищем индекс этого хода в массиве correctMoves
        let moveIndex1 = correctMoves.findIndex(correctMove => correctMove === move);
        if (moveIndex1 >= 0) {
            setMoveIndex(prevIndex => prevIndex + 1);
            // Если ход найден
            if (moveIndex1 % 2 === 0 && moveIndex1 - moveIndex == 1 ) {
                console.log("AI move");
                makeAIMove();
            } else if (moveIndex1 - moveIndex == 1){
                console.log("User move");
                handleUserMove(orig, dest);
            }
        } else {
            console.log('wrong user move');
